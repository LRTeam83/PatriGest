...
import { financialAccountLabels, formatCurrency, formatFinancialDate, getCurrentAccountValue } from "@/domains/financial-accounts/utils/financial-account-utils";
...
export default async function FinancialAccountsPage({ params }: { params: Promise<{ protectedPersonId: string }> }) {
  ...
  const total = active.reduce((sum, account) => sum + getCurrentAccountValue(account, account.valuations).value, 0);
  return <PrivateShell current="dossiers"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-[#2563EB]">{person.first_name} {person.last_name}</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Comptes et patrimoine</h1></div><Link href={`/dossiers/${protectedPersonId}/comptes/nouveau`} className="button button-primary gap-2"><Plus aria-hidden="true" size={18} />Ajouter un compte</Link></div><DossierNavigation protectedPersonId={protectedPersonId} current="accounts" />
