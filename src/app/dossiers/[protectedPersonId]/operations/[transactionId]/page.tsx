...
import { getSafeTransactionReturnTo, withTransactionReturnTo } from "@/domains/transactions/return-to";

export const metadata: Metadata = { title: "Consulter l’opération" };
export const dynamic = "force-dynamic";

export default async function OperationDetailPage({ params, searchParams }: { params: Promise<{ protectedPersonId: string; transactionId: string }>; searchParams: Promise<{ returnTo?: string | string[] }> }) {
  ...
  const returnHref = getSafeTransactionReturnTo(protectedPersonId, typeof requestedReturnTo === "string" ? requestedReturnTo : undefined, item.financial_account_id);
  if (!item.transfer_id) redirect(withTransactionReturnTo(`/dossiers/${protectedPersonId}/operations/${transactionId}/modifier`, returnHref));
