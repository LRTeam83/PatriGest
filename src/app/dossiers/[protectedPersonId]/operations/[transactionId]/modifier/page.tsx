...
export default async function EditOperationPage({ params }: { params: Promise<{ protectedPersonId: string; transactionId: string }> }) {
  ...
  return <PrivateShell current="dossiers" dossier={{ id: protectedPersonId, name: `${person.first_name} ${person.last_name}`, current: "operations" }}>
    ...
    {locked ? <section className="mt-5 rounded-xl border border-[#E2E8F0] bg-white p-6 text-center"><span className="mx-auto flex size-9 items-center justify-center rounded-lg bg-slate-100 text-[#64748B]"><LockKeyhole size={17} /></span><h2 className="mt-3 text-base font-bold">{closedPeriod ? "Exercice clôturé" : "Consultation en lecture seule"}</h2><p className="mx-auto mt-1 max-w-xl text-xs text-[#64748B]">{closedPeriod ? "Cette opération appartient à un exercice clôturé et ne peut plus être modifiée." : "Vous pouvez consulter cette opération, mais vous ne pouvez pas la modifier."}</p><Link href={`/dossiers/${protectedPersonId}/operations`} className="button button-secondary mt-4">Retour aux opérations</Link></section> : <section className="mt-5 max-w-4xl rounded-xl border border-[#E2E8F0] bg-white p-4 sm:p-5"><TransactionForm personId={protectedPersonId} accounts={accounts} categories={categories} transaction={transaction} /></section>}
