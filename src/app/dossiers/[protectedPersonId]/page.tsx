...
export default async function ProtectedPersonDetailPage({ params }: { params: Promise<{ protectedPersonId: string }> }) {
  ...
  return <PrivateShell current="dossiers">
    <AppBreadcrumb items={[{ label: "Tableau de bord", href: "/tableau-de-bord" }, { label: `${person.first_name} ${person.last_name}` }]} />
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <InfoCard icon={WalletCards} title="Patrimoine actuel"><p className="text-3xl font-bold">{formatCurrency(currentPatrimony)}</p><Link href={`/dossiers/${protectedPersonId}/comptes`} className="auth-link mt-3 inline-block text-sm">Voir les comptes</Link></InfoCard>
      ...
      <InfoCard icon={CalendarDays} title="Exercice de gestion">{openPeriod ? <><p className="font-semibold">Exercice ouvert</p><p className="mt-2 text-sm text-[#64748B]">Du {formatDate(openPeriod.start_date)} au {formatDate(openPeriod.end_date)}</p></> : <p className="text-[#64748B]">Aucun exercice ouvert</p>}<Link href={`/dossiers/${protectedPersonId}/exercices`} className="button button-secondary mt-5">Gérer les exercices</Link></InfoCard>
    </div>
  </PrivateShell>;
}
