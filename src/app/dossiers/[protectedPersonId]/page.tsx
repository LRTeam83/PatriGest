...
export default async function ProtectedPersonDetailPage({ params }: { params: Promise<{ protectedPersonId: string }> }) {
  ...
  const currentPatrimony = accounts.filter((account) => account.status === "active").reduce((total, account) => total + getCurrentAccountValue(account, account.valuations).value, 0);
