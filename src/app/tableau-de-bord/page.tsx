...
export default async function DashboardPage() {
  ...
  const pendingReport = data.pendingReportPeriods[0];
  return <PrivateShell current="dashboard">
    ...
    {pendingReport && <Link href={`/dossiers/${pendingReport.protected_person_id}/comptes-de-gestion`} className="focus-ring mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3"><span><strong className="text-sm">Compte de gestion {pendingReport.end_date.slice(0,4)}</strong><span className="ml-2 text-xs text-[#64748B]">Votre exercice est clôturé.</span></span><span className="text-xs font-bold text-[#2563EB]">Préparer</span></Link>}
