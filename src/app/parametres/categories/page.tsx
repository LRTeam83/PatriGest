...
export default async function CategoriesPage() { const categories = await getCategories(); return <PrivateShell current="settings"><AppBreadcrumb items={[{ label: "Tableau de bord", href: "/tableau-de-bord" }, { label: "Paramètres", href: "/parametres/categories" }, { label: "Catégories" }]} /><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#2563EB]">Paramètres</p><h1 className="mt-1 text-2xl font-bold sm:text-[28px]">Catégories</h1><p className="mt-1 text-sm text-[#64748B]">Classez vos recettes et dépenses avec les catégories PatriGest ou les vôtres.</p></div><CategoryManager categories={categories} /></PrivateShell>; }
