...
function Card({ item, personId, closed }: { item: TransactionJournalItem; personId: string; closed: boolean }) {
  ...
  return <article className="rounded-xl border border-[#E2E8F0] bg-white p-3.5"><div className="flex justify-between gap-3"><div><p className="text-sm font-bold">{item.label}</p><p className="text-[11px] text-[#64748B]">{formatFinancialDate(item.transaction_date)} · {item.account.account_name}</p></div><p className={`text-sm font-bold ${positive ? "text-[#16A34A]" : "text-[#DC2626]"}`}>{positive ? "+" : "−"}{formatCurrency(item.amount)}</p></div><div className="mt-2.5 flex items-center justify-between gap-2"><span className="text-[11px] text-[#64748B]">{transactionTypeLabels[item.transaction_type]} · {item.category?.name ?? "Sans catégorie"}</span>{closed ? <ClosedBadge /> : <Actions item={item} personId={personId} />}</div></article>;
}
