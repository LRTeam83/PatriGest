...
export function TransactionForm({ personId, accounts, categories, transaction }: { personId: string; accounts: FinancialAccount[]; categories: Category[]; transaction?: Transaction }) {
  ...
  return <form action={formAction} className="grid gap-3 sm:grid-cols-2">
    ...
    {mode === "transfer" && !transaction ? <>
      ...
      <AccountSelect name="financialAccountId" label="Compte" accounts={accounts} defaultValue={transaction?.financial_account_id} errors={state.fieldErrors?.financialAccountId} />
      ...
      <Field name="proofReference" label="Référence justificatif facultative" defaultValue={transaction?.proof_reference} />
